function [imga imgb imsnr] = get_dist_gre(mag,phase,magicn,nex,matrix,bw,b0,phasencode,epi,gray_pix,gaussopt)
fov=256;
res=(fov/matrix);
slicet=.1;
if (epi=='onn')
...
r1=noise*randn(matrix1);
 r2=noise*randn(matrix1);
  rn=r1+i*r2;
...
if (gaussopt=='onn')
   ...
   b = b .* gauss;
%    figure('Name','Kspace after gauss filter 1');
%     b = b .* gauss;
%    figure('Name','Kspace after gauss filter 2');
%    imagesc(abs(b).^.2);
%    colormap(gray);
end 
c = angle(nks_comp);
